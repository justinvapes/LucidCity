...
function ResetClothing(anim)
	isInJailClothes = exports["lcrp-jail"]:isInJailClothes()
	if isInJailClothes then return Notify("You can\'t take off jail clothes!", "error") end
	local Ped = PlayerPedId()
	...
	TriggerEvent("dpclothing:ResetClothing")
end
...
function ToggleClothing(which, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify(Lang("NotAllowedPed"), "error") return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify(Lang("NoVariants"), "error") return
	else
		if not LastEquipped[which] then
	...
	Notify(Lang("AlreadyWearing"), "error") return false
end
...
function ToggleProps(which)
	...
	if not Prop.Variants then
		...
		Notify(Lang("NothingToRemove"), "error") return false
	else
		local Gender = IsMpPed(Ped)
		...
		Notify(Lang("NoVariants"), "error") return false
	end
end
...
function DevTestVariants(d) -- If debug mode is enabled we can try all the variants to check for scuff.
	if not TestThreadActive then
		...
		Notify("Already testing variants.", "error")
	end
end
