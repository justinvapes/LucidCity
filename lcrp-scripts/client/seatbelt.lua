    ...
    while true do
        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
            ...
            if IsDisabledControlJustPressed(0, 75) or IsDisabledControlJustPressed(27, 75) then
                if seatbeltOn then
                    scCore.Notification("You can\'t exit vehicle with seatbelt on!", "error")
                end
            end
        else
            sleep = 500
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
            ...
            thisFrameVehicleSpeed = GetEntitySpeed(currentVehicle) * 2.23694
            ...
            if tick > 0 then 
                tick = tick - 1
                if tick == 1 then
                    lastFrameVehiclespeed = GetEntitySpeed(currentVehicle) * 2.23694
                end
            else
                if damagedone then
                    damagedone = false
                ...
                lastFrameVehiclespeed2 = GetEntitySpeed(currentVehicle) * 2.23694
