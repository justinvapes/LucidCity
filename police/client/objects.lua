...
AddEventHandler('police:client:spawnCone', function()
    scCore.TaskBar("spawn_object", "Placing an object..", 2500, false, true, {
...
AddEventHandler('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        scCore.TaskBar("remove_object", "Deleting object..", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        ...
        scCore.Notification('There are no more Spikestrips left', 'error')
    end
end)
