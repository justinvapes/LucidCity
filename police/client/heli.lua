...
-- Script starts here
local helicam = false
-- local pdHeli_hash = GetHashKey("maverick")
local fibHeli_hash = GetHashKey("frogger")
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if isLoggedIn then
			if PlayerJob.name == 'police' or PlayerJob.name == 'fib' or PlayerJob.name == 'reporter' and onDuty then
				if IsPlayerInPolmav() then
					...
					if helicam then
						...
						while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
							...
							if IsControlJustPressed(0, toggle_vision) and (PlayerJob.name == 'police' or PlayerJob.name == 'fib')then
								ChangeVision()
							end
							...
							if locked_on_vehicle then
								if DoesEntityExist(locked_on_vehicle) then
									...
									if IsControlJustPressed(0, toggle_lock_on) and (PlayerJob.name == 'police' or PlayerJob.name == 'fib') then
										locked_on_vehicle = nil
...
function IsPlayerInPolmav()
	...
	-- return IsVehicleModel(vehicle, pdHeli_hash) or IsVehicleModel(vehicle, fibHeli_hash)
	return IsVehicleModel(vehicle, fibHeli_hash)
end
...
function RenderVehicleInfo(vehicle)
	...
	local speed = math.ceil(GetEntitySpeed(vehicle) * 2.23694)
