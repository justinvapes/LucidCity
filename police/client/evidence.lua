StatusList = {
	...
	["heavybreath"] = "Heavy breath",
	["sweat"] = "Body Sweat",
    ["handbleed"] = "Blood on hand",
	["confused"] = "Confused",
	["alcohol"] = "Breath smells like Alcohol",
	["heavyalcohol"] = "Heavy smell of alcohol",
	["saturatedclothing"] = "Saturated Clothing",
	["dazed"] = "Looks Dazed",
...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	scCore.TaskBar("clear_blooddrops", "Clearing blood drops", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			scCore.Notification("Cleared blood")
		end
    end, function() -- Cancel
...
AddEventHandler('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
		type = weapon,
		serie = serie ~= nil and serie or "Serial number is not visible",
		coords = {
			x = coords.x,
			y = coords.y, 
			z = coords.z - 0.9,
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(GetPlayerPed(-1))
	local casingList = {}
	scCore.TaskBar("clear_casings", "Removing casings", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			scCore.Notification("Bullet casings cleared")
		end
    end, function() -- Cancel
        scCore.Notification("Canceled", "error")
    end)
		...
		if IsPedShooting(GetPlayerPed(-1)) then
			...
			if weapon ~= GetHashKey("WEAPON_SNOWBALL") and weapon ~= GetHashKey("WEAPON_STUNGUN") and weapon ~= GetHashKey("WEAPON_PETROLCAN") and weapon ~= GetHashKey("WEAPON_FIREEXTINGUISHER") then
				...
				if shotAmount > 5 then
					...
					local ShootingSkillCount = math.random(1,10)
					if ShootingSkillCount > 2 then
						TriggerServerEvent("player:UpdateSkill", "add", "shooting", 1)
						shotAmount = 0
					end
				end
	...
	while true do 
		...
		if CurrentCasing ~= nil and CurrentCasing ~= 0 then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, "~r~[G]~w~ - Pickup Bullet casing ~r~#"..Casings[CurrentCasing].type)
				if IsControlJustReleased(0, Keys["G"]) then
					...
					local info = {
						...
						ammolabel = Config.AmmoLabels[scCore.Shared.Weapons[Casings[CurrentCasing].type]["ammotype"]],
		...
		if CurrentBlooddrop ~= nil and CurrentBlooddrop ~= 0 then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, true) < 1.5 then
				DrawText3D(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, "~r~[G]~w~ - Blood drops ~r~#"..DnaHash(Blooddrops[CurrentBlooddrop].citizenid))
				if IsControlJustReleased(0, Keys["G"]) then
		...
		if CurrentFingerprint ~= nil and CurrentFingerprint ~= 0 then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, true) < 1.5 then
				DrawText3D(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, "~r~[G]~w~ - Fingerprint ")
				if IsControlJustReleased(0, Keys["G"]) then
			...
			if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_FLASHLIGHT") then
				if PlayerJob.name == "police" or PlayerJob.name == "statepolice" or PlayerJob.name == "fib" and onDuty then
					for k, v in pairs(CasingsNear) do
						if v ~= nil then
							DrawMarker(27, v.coords.x, v.coords.y, v.coords.z - 0.05, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, 0.11, 0.11, 0.3, 30, 30, 220, 255, false, true, 2, false, false, false, false)
						end
					end
				end
			else
		...
		if isArmed then
			if PlayerJob.name ~= "police" or PlayerJob.name ~= "statepolice" then
				if IsPedShooting(GetPlayerPed(-1)) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
					TriggerEvent("lcrp-weazelnews:client:CrimeStatistics", "shotsFired", 1)
					local coords = GetEntityCoords(GetPlayerPed(-1))
					local automatic = false
					if scCore.Shared.Weapons[currentWeapon]["ammotype"] ~= "AMMO_PISTOL" then
						automatic = true
					end
...
function IsSilentWeapon(weapon)
	...
	if not retval then 
		scCore.TriggerServerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)
