...
function Process(action, start, tick, finish)
    newAction = action
    if not IsEntityDead(PlayerPedId()) or newAction.useWhileDead then
        if not isDoingAction then
            ...
            Citizen.CreateThread(function ()
                ...
                while isDoingAction do
                    ...
                    if IsControlJustPressed(0, Keys["X"]) and Action.canCancel then
            ...
            scCore.Notification("You're already doing something!", "error")
        end
    else
        scCore.Notification("Can't take action!", "error")
    end
end
...
function DisableActions(ped)
    ...
    if Action.controlDisables.disableCancel then
        DisableControlAction(0, 73, true) -- VehicleMouseControlOverride
    end
