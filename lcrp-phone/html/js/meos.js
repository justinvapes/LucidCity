...
SetupMeosHome = function() {
    $("#meos-app-name").html("Welkom " + scPhone.Data.PlayerData.charinfo.firstname + " " + scPhone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('http://lcrp-phone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('http://lcrp-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    scPhone.Notifications.Add("fas fa-university", "MEOS", "House No. copied!", "#badc58", 1750);
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://lcrp-phone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                scPhone.Notifications.Add("politie", "MEOS", "There are no search results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://lcrp-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://lcrp-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('http://lcrp-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Ja";
            if (!vehicle.status) {
            ...
            scPhone.Notifications.Add("politie", "MEOS", "No vehicle around!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        $.post('http://lcrp-phone/SetAlertWaypoint', JSON.stringify({
            alert: alertData,
        }));
    } else {
        scPhone.Notifications.Add("politie", "MEOS", "This message has no GPS Location!");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    scPhone.Notifications.Add("politie", "MEOS", "All notifications have been removed!");
});*/