...
$(document).on('click', '.cryptotab-general-action', function(e){
    ...
    scPhone.Functions.HeaderTextColor("black", 300);
    ActionTab = Tab;
});
...
$(document).on('click', '#cancel-crypto', function(e){
    ...
    scPhone.Functions.HeaderTextColor("white", 300);
});
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (scPhone.Data.PlayerData.money.bank >= Price) {
            $.post('http://lcrp-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    scPhone.Data.PlayerData.money.bank = parseInt(scPhone.Data.PlayerData.money.bank) - parseInt(Price);
                    scPhone.Notifications.Add("fas fa-university", "QBank", "&dollar; "+Price+",- written off!", "#badc58", 2500);
                } else {
                    scPhone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#badc58", 1500);
                }
            });
        } else {
            scPhone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#badc58", 1500);
        ...
        scPhone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill in everything!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('http://lcrp-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    scPhone.Data.PlayerData.money.bank = parseInt(scPhone.Data.PlayerData.money.bank) + parseInt(Price);
                    scPhone.Notifications.Add("fas fa-university", "QBank", "&dollar; "+Price+",- credited!", "#badc58", 2500);
                } else {
                    scPhone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#badc58", 1500);
                }
            });
        } else {
            scPhone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://lcrp-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        scPhone.Notifications.Add("fas fa-university", "Crypto", "This Wallet ID does not exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        scPhone.Notifications.Add("fas fa-university", "Crypto", ""+Coins+",- transferred to "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                scPhone.Notifications.Add("fas fa-university", "Crypto", "You cannot transfer to yourself..", "#badc58", 2500);
            }
        } else {
            scPhone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#badc58", 1500);
        }
