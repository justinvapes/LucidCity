...
AddEventHandler("scCore:client:LoadInteractions", function()
	exports["lcrp-interact"]:AddBoxZone("golfToggle", vector3(-1347.02, 143.67, 56.53), 3.2, 2.8, {
		name="golfToggle",
		heading=7,
		--debugPoly=true,
		minZ=55.13,
        ...
        options = {
            {
                event = "lcrp-activities:client:playGolf",
...
RegisterNetEvent("lcrp-activities:client:playGolf")
AddEventHandler("lcrp-activities:client:playGolf", function()
	if not golfplaying then
		TriggerServerEvent("lcrp-activities:server:payGolf")
		TriggerEvent("customNotification","Press G to swing, A-D to rotate, Y to swap club.")
		local options = {
			{
				event = "lcrp-activities:client:endGolfGame",
				icon = "fas fa-golf-club",
				label = "End golf game",
				duty = false,
			}
		}
		TriggerEvent("lcrp-interact:client:updateInteractionOptions", 'zone', 'golfToggle', options)
	end
end)

RegisterNetEvent("lcrp-activities:client:endGolfGame")
AddEventHandler("lcrp-activities:client:endGolfGame", function()
	if golfplaying then
		endgame()
...
function isGolfCartVeh(vehicle)
    local retval = false
	if GetEntityModel(vehicle) == GetHashKey("caddy") then
		retval = true
	end
    return retval
end

RegisterNetEvent("lcrp-activities:client:SpawnCart")
AddEventHandler("lcrp-activities:client:SpawnCart", function()
	local vehicle = GetHashKey("caddy")
	RequestModel(vehicle)
	while not HasModelLoaded(vehicle) do
		Citizen.Wait(0)
	end
	spawned_car = CreateVehicle(vehicle, -1350.71, 136.1, 56.26, 180, true, false)

	exports['LegacyFuel']:SetFuel(spawned_car, 100.0)
...
RegisterNetEvent("lcrp-activities:client:StartGolf")
AddEventHandler("lcrp-activities:client:StartGolf", function()
	--endgame()
	...
	Citizen.CreateThread(function()
		local playerLoc = GetEntityCoords(GetPlayerPed(-1))
		local distance = GetDistanceBetweenCoords(-1332.78,128.18,57.03, playerLoc.x,playerLoc.y,playerLoc.z, true)
		while golfplaying and distance < 500 do
			playerLoc = GetEntityCoords(GetPlayerPed(-1))
			distance = GetDistanceBetweenCoords(-1332.78,128.18,57.03, playerLoc.x,playerLoc.y,playerLoc.z, true)
			Citizen.Wait(1000)
		end
		if golfplaying then
			endgame()
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				scCore.ShowHelpNotification("Move to your ball, press ~g~G~s~ to ball drop if you are stuck.")
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			scCore.ShowHelpNotification("Press ~g~G~s~ to drop here.")
			if ( IsControlJustReleased(1, 47) ) then
				doingdrop = false
			...
			scCore.ShowHelpNotification("Press ~g~E~s~ to drop - ~r~ too far from ball or to close to hole.")
		end
	end
end
...
AddEventHandler('customNotification', function(response)
	TriggerEvent("chatMessage", "GOLF", "warning", response)
end)
