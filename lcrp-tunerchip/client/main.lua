...
RegisterNUICallback('save', function(data)
    scCore.TriggerServerCallback('lcrp-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            scCore.Notification('Vehicle Tuned!', 'error')

            TriggerServerEvent('lcrp-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('lcrp-tunerchip:server:TuneStatus')
AddEventHandler('lcrp-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    scCore.TriggerServerCallback('lcrp-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    scCore.Notification('Vehicle has been reset!', 'error')
end)

RegisterNetEvent('lcrp-tunerchip:client:openChip')
AddEventHandler('lcrp-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        scCore.TaskBar("connect_laptop", "Connecting", 2000, false, true, {
