...
AddEventHandler('scCore:Client:OnPlayerLoaded', function()
    scCore.TriggerServerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            scCore.TaskBar("use_nos", "connecting NOS", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", scCore.Shared.Items['nitrous'], "remove")
                TriggerServerEvent("scCore:Server:RemoveItem", 'nitrous', 1)
                local CurrentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
                local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
            end)
        else
            scCore.Notification('You\'re not in a car', 'error')
        end
    else
        scCore.Notification('You already have NOS active', 'error')
    end
end)
...
--                         local speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 2.23694
...
--                                     TriggerEvent('lcrp-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, true)
--                                 else
...
--                             end
--                             StopScreenEffect("RaceTurbo")
--                             TriggerEvent('lcrp-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, false)
--                             NitrousActivated = false
--                         end
--                     end
--                 end
...
--                     TriggerEvent('lcrp-hud:client:UpdateNitrous', false, nil, false)
--                     nosupdated = true
--                 end
--             end
--         else
