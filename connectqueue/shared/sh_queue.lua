...
Queue.MaxPlayers = GetConvarInt('sv_maxclients', 350)
...
function Queue:IsPriority(ids)
    ...
    for k, v in ipairs(ids) do
        local codename = string_sub(v, 1, 5)
        if codename == "steam" then
            steam = v
        elseif codename == "fivem" then
            fivem = v
        end
        if steam and fivem then
            break
        end
    end
    if fivem ~= nil then
        if Config.Fivem[fivem] ~= nil then
            if Config.Fivem[fivem] then
                if prioList[steam] then
                    prio = prioList[Config.Fivem[fivem]]
                end
            else
                prio = LinkSteamFivem(fivem, steam)
                Config.Priority[steam] = prio
            end
        end
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(Config.Language.connectingerr .."\n📢 Check out our discord for more information: discord.gg/lucidcity")
...
function CheckFivem(ids)
    local hasFivem = false
    for k, v in ipairs(ids) do
        local codename = string_sub(v, 1, 5)
        if codename == "fivem" then
            hasFivem = true
            break
        end
    end
...
function Queue:NotFull(firstJoin)
    print("Con ".. Queue:ConnectingSize())
    print("Players ".. Queue:GetPlayerCount())
...
local function playerConnect(source, setKickReason, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() and Queue:ConnectingSize() < 30 then
        -- let them in the server
        local added = Queue:AddToConnecting(ids, true, true, done)
        if not added then CancelEvent() return end

        done()
        -- TriggerEvent("lcrp-logs:server:CreateLog", "joinleave", "Queue", "orange", "".. name .. " is loading into the server")
        -- TriggerEvent("lcrp-logs:server:sendLog", ids[1], "joinedqueue", {})
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and _Queue.JoinDelay <= GetGameTimer() and Queue:ConnectingSize() < 30 then
            -- let them in the server
        ...
        data.hasFivem = CheckFivem(data.ids)
        if data.hasFivem then
            msg = msg .. "\n FiveM account: LINKED ✔️"
        else
            msg = msg .. "\n FiveM account: NOT LINKED ❌"
        end
        if data.priority then
            if data.priority == 99 then
                msg = msg .. "\nPriority: Temporary reconnect prio 🎉"
            else
                msg = msg .. "\nPriority: ON 🎉"
            end
        else 
            msg = msg .. "\nPriority: OFF 🐌\n ⚠️If you should have queue prio, close the game, wait 5 minutes, re-join queue⚠️"
        end
        msg = msg .. "\n\n 🚀 For queue prio check https://store.lucidcityrp.com/"
        update(msg, data.deferrals)
    end
end
...
RegisterCommand("removeq", function(src, args)
    commands.removeq(args[1])
end, true)

commands.removeq = function(remove)
    remove = tonumber(remove)
    local name = Queue:GetQueueList()[remove] and Queue:GetQueueList()[remove].name or nil
    Queue:RemoveFromQueue(nil, nil, remove)
    Queue:DebugPrint("REMOVED " .. tostring(name) .. " FROM THE QUEUE")
end

RegisterCommand("checkq", function()
    commands.printq()
end, true)
...
RegisterCommand("checkcon", function()
    commands.printc()
end, true)
...
RegisterCommand("qsetpos", function(cur, pos)
    commands.printc(pos[1], pos[2])
end, true)
...
function LinkSteamFivem(fivem, steam)
    local prio = false
    local processed = false
    exports['ghmattimysql']:execute("SELECT priority FROM queue WHERE fivem = @fivem", {
        ['@fivem'] = fivem
    }, function(result)
        if result ~= nil then
            if result[1] ~= nil then
                if result[1].priority ~= nil then
                    prio = result[1].priority
                    exports['ghmattimysql']:execute("UPDATE queue SET steam = @steam WHERE fivem = @fivem", {
                        ['@fivem'] = fivem,
                        ['@steam'] = steam,
                    })
                    processed = true
...
Citizen.CreateThread(function()
    Citizen.Wait(120000 + 30000) -- Queue join delay + 2 minute of server crying until gets good
    loopWait = 4500
end)
