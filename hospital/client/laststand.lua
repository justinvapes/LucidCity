        ...
        Citizen.CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    scCore.Notification("You bled out", "error")
                    SetLaststand(false)
...
AddEventHandler('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        scCore.Notification('Action not possible!', 'error')
    end
end)
...
AddEventHandler('hospital:client:HelpPerson', function(targetId)
    ...
    scCore.TaskBar("hospital_revive", "Helping person", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        scCore.Notification("You helped the person up.")
