...
whitelisted["male"] = {
    jackets:[4,7,8,14,18,19,52,107,143,72,73,77,86,113,123,141,149,186,198,199,201,246,259,277,278,286,118,148,165,178,255,228, 260, 358, 357],
    undershirts:[16,17,19,20,22,24,25,26,28,29,33,34,35,36,39,40,42,57,58,102,115,116,166,173,174],
    hats:[4,20,21,10],
    shoes:[1,3,11,13,15,31,51, 84, 85],
    legs:[4,5,11,29,42,90, 101, 131, 1],
    glasses:[1,4,19,25,30],
    neck:[3,5,7,11,15,130],
    bags:[52,54,65,34],
    hair:[42]
}

whitelisted["female"] = {
    jackets:[13,17,18,21,26,28,65,70,73,84,110,116,111,125,140,145,152,171,179,200,236,237,239,248,253,254,134,142,144,197,198,263,264,280,285,291,296,322,323, 33, 34, 35, 69, 106, 132, 250, 289],
    //undershirts:[],
    pants:[9, 16, ,20, 24,25,26,73,74,79,87,96,102,81,93,104,106,108,12,16,17,22,24,43,78,98],
    hats:[15,20,39],
    //vest:[8,9,11,12,13,14,15,16,17,18,19,21,22],
    shoes:[3,10,4,8,11,15,23,20,48,90, 30, 88, 89, 98],
    pants:[20,21,38,33,34,35,37,51,62,69,72,63,87],
    glasses:[0,7,27,32],
    neck:[7,11],
    bags:[52,54,65],
    hair:[3,5]
...
/*
private["male"] = {
    jackets:[81, 2, 92, 95, 101, 102, 108, 110, 159, 160, 249, 250,254],
    glasses:[27],
    legs:[101]
}

private["female"] = {
    jackets:[14,48,74,86,92,93,99],
    glasses:[29],
}
*/
...
$(function () {
    ...
    function CloseMenu(save) {
        $.post('http://lcrp-clothes/escape', JSON.stringify({save:save}));
    }
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://lcrp-clothes/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if(($('#skin_female').val() == 1 || $('#skin_male').val() == 1)) {
                ...
                if (isVip) {
                    if(ele.is(inputs.eq(0))){
                        if (whitelisted[gender][clothingName] != null && whitelisted[gender][clothingName].length > 0){
                            while (isNotValid){
                                if (private[gender][clothingName] != null){
                                    if(whitelisted[gender][clothingName].indexOf(clothingID) > -1 && private[gender][clothingName].indexOf(clothingID) < 0){
                                        isNotValid = false 
                                    }
                                    else
                                    {
                                        ...
                                        if (clothingID > whitelisted[gender][clothingName][whitelisted[gender][clothingName].length - 1]) {
                                            clothingID = whitelisted[gender][clothingName][0]
                                        }else if (clothingID < whitelisted[gender][clothingName][0]){
                                            clothingID = whitelisted[gender][clothingName][whitelisted[gender][clothingName].length - 1]
                                        }
                                    }
                                }else {
                    ...
                    if(ele.is(inputs.eq(0)) && whitelisted[gender][clothingName]){
                        while (isNotValid) {
                            if (private[gender][clothingName] != null && private[gender][clothingName].length > 0){
                                if(whitelisted[gender][clothingName].indexOf(clothingID) > -1 || private[gender][clothingName].indexOf(clothingID) > -1){
                                    isNotValid = true
                                    if(inputType){clothingID++;}else{clothingID--;}
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('http://lcrp-clothes/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('http://lcrp-clothes/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('http://lcrp-clothes/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://lcrp-clothes/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://lcrp-clothes/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://lcrp-clothes/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://lcrp-clothes/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://lcrp-clothes/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('http://lcrp-clothes/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('http://lcrp-clothes/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('http://lcrp-clothes/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('http://lcrp-clothes/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('http://lcrp-clothes/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://lcrp-clothes/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://lcrp-clothes/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
    ...
    function SetupTatValues(data) {
        ...
        keys.forEach((key,index)=>{
            $('#'+key).find('.input-number').val(index)
        })
    }
});
