...
RegisterCommand("control_handsup", function(source)
  if not handsup then
    handsup = true
...
RegisterCommand("control_animationcancel", function(source)
    handsup = false
    EmoteCancel()
...
AddEventHandler('animations:client:SmokeWeed', function()
  ...
  Citizen.CreateThread(function()
    while SmokingWeed do
      Citizen.Wait(10000)
      TriggerServerEvent('lcrp-hud:Server:RelieveStress', math.random(15, 18))
...
AddEventHandler('animations:client:EmoteCommandStart', function(args, source, isCommand)
  if CanDoEmote then
    EmoteCommandStart(args, source, isCommand)
  else
    scCore.Notification("You cannot do emotes at this time", "error")
  end
end)
...
AddEventHandler('animations:client:EmoteCommandStop', function()
  EmoteCancel()
end)
...
function EmoteCommandStart(args, source, isCommand)
    if #args > 0 then
      local name = string.lower(args[1])
      if source ~= nil and source ~= 0 then
        for k, v in pairs(itemBoundEmote) do
          if v == name then
            return
          end
        end
      ...
      if AnimationList.Emotes[name] ~= nil then
        if OnEmotePlay(AnimationList.Emotes[name], isCommand) then end return
      elseif AnimationList.Dances[name] ~= nil then
        if OnEmotePlay(AnimationList.Dances[name], isCommand) then end return
      elseif AnimationList.PropEmotes[name] ~= nil then
        if OnEmotePlay(AnimationList.PropEmotes[name], isCommand) then end return
      end
    end
end
...
function OnEmotePlay(EmoteName, isCommand)
  ...
  if ChosenDict == "MaleScenario" or "Scenario" then
    ...
    if EmoteName.AnimationOptions ~= nil then 
      if EmoteName.AnimationOptions.AllowedCommand == nil then 
        EmoteName.AnimationOptions.AllowedCommand = true
      end

      if not EmoteName.AnimationOptions.AllowedCommand and isCommand then return end
    end
