...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					scCore.Notification("You have no ammunition.")
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			scCore.Notification("You can't hostage anyone with this weapon")
			return
		end
		...
		scCore.Notification("There's no one around")
	end
...
Citizen.CreateThread(function()
	while true do 
		...
		if holdingHostage then
			...
			showSubtitle("~g~[G]~w~ Release hostage. ~r~[H]~w~ Shoot hostage.")
