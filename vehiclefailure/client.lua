...
function CleanVehicle(vehicle)
	...
	scCore.TaskBar("repair_vehicle", "Cleaning vehicle", math.random(8000, 12000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		scCore.Notification("Vehicle cleaned!")
		SetVehicleDirtLevel(vehicle)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('vehiclefailure:server:removewashingkit')
		TriggerEvent('inventory:client:ItemBox', scCore.Shared.Items["cleaningkit"], "remove")
...
AddEventHandler('vehiclefailure:client:CleanVehicle', function()
	...
	if vehicle ~= nil and vehicle ~= 0 then
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 3.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			scCore.Notification("No vehicle nearby", "error")
		end
	end
end)
...
RegisterNetEvent('vehiclefailure:client:RepairVehicleMechanic')
AddEventHandler('vehiclefailure:client:RepairVehicleMechanic', function()
	local vehicle = scCore.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		local pos = GetEntityCoords(GetPlayerPed(-1))
		local vehpos = GetEntityCoords(vehicle)
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, drawpos) < 2.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
				RepairVehicleMechanicFull(vehicle)
			else
				scCore.Notification("Move closer to vehicle\'s hood!", "error")
				ShowEnginePos = true
			end
		end
	end
...
function RepairVehicleMechanicFull(vehicle)
	if (IsBackEngine(GetEntityModel(vehicle))) then
        SetVehicleDoorOpen(vehicle, 5, false, false)
    else
        SetVehicleDoorOpen(vehicle, 4, false, false)
    end
	scCore.TaskBar("repair_vehicle", "Repairing vehicle", math.random(8000, 12000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		scCore.Notification("Vehicle repaired!")
...
function RepairVehicleFull(vehicle)
	...
	scCore.TaskBar("repair_vehicle", "Repairing vehicle", math.random(6000, 10000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('lcrp-vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		scCore.Notification("Canceled!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	...
	scCore.TaskBar("repair_vehicle", "Repairing vehicle", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('lcrp-vehiclefailure:removeItem', "repairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		scCore.Notification("Canceled!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
--         if showPro then
--             scCore.Draw3DText(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
--         end
--     end
-- end)
...
-- 								--exports['lcrp-tuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['lcrp-tuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
-- 							end
